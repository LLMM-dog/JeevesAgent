...
sub.pending_future` 没有 timeout）、pi 永久占并发槽位。
...
为什么需要两道 —— 白名单是**配置**，配置会被写错。pi 只靠白名单，它的
...
#
# 50KB 是 pi 试出来的折中值。100 字符太少（模型看不懂发生了什么），
# 无上限等于没做委派。
...
class SubAgentTool:
    ...
    description = (
        ...
        "\n"
        "## 什么时候用\n"
        "- 任务需要读大量材料（十几个文件、多个网页）而你只要结论\n"
        "- 任务可以独立完成，不依赖当前对话的推理过程\n"
        "- 需要一个受限权限的执行者（比如只读审查）\n"
        "\n"
        "## 什么时候不要用\n"
        "- 任务简单，你自己两三步就能做完 —— 委派本身有开销，杀鸡不用牛刀\n"
        "- 任务强依赖当前上下文里的推理 —— 子智能体看不到这些，说不清就做不对\n"
        "- 只是想'并行提速' —— 子智能体有并发上限，且结果要等齐\n"
        "\n"
        "## task 必须自包含\n"
        "子智能体**看不到当前对话**，只看到你写的 task。\n"
        "不要写「按上面说的做」「继续刚才的任务」「参考前面的分析」——\n"
        "对方没有'上面'。必要的背景、文件路径、约束都要写进 task。\n"
        "\n"
        "返回的是子智能体的最终结论。中间过程不回给你（那是省上下文的目的）。"
    )
    # 委派本身不需要审批 —— 子智能体内部的危险操作会各自触发审批。
    # 在这里再拦一层等于同一件事问两次。

...
启动脚本放在**项目根目录**而不是 `scripts/` —— 它是最常用的入口，放根目录能直接 `.\start.ps1` 而不用记路径。`scripts/` 下放的是 `setup.py` 和各个 `verify_*.py`（都是偶尔跑一次的）。
...
如果路径不对，说明 `config.py` 里的 `parents[N]` 数字与实际目录层级不匹配。见 [../02-data/files.md](../02-data/files.md#env-路径必须绝对)。
...
`TextDecoder.decode()` 没传 `{ stream: true }`。见 [../04-frontend/sse.md](../04-frontend/sse.md#三个必须注意的细节)。

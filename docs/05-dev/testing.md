...
def test_all_emitted_events_are_documented():
    ...
    与 docs/03-api/sse-events.md 里的表对比，两边必须完全一致。
    多了或少了都失败。
    """
```
...
前端侧：`EventHandlers` interface 的所有字段必需（不用 `?`），后端加事件时 TS 编译报错。见 [../04-frontend/sse.md](../04-frontend/sse.md#dispatch)。
...
以及 base_url 规范化的参数化测试（表见 [../01-architecture/providers.md](../01-architecture/providers.md#base_url-规范化)）。
...
每个路线图阶段的验收标准（见 ../00-overview/路线图）就是手动测试清单。

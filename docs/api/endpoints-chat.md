...
`private_mode` / `amnesia_mode` 是两件事：私密是"别记住我说的"（拦记忆写入），失忆是"别拿以前的事来烦我"（拦记忆召回）。拆成两个开关是因为用途不同 —— 调试提示词时要失忆但不介意被记住，聊敏感话题时要私密但仍需要之前的上下文。
...
多返回一个猜测的进度比不返回更糟。要真支持重连得让 EventBus 支持多消费者 + 事件重放缓冲，见 [../architecture/events.md](../architecture/events.md#detach没人听的时候不能阻塞)。

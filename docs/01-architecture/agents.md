...
为什么要两道 —— 白名单是**配置**，配置会被写错。pi 只靠白名单，它的 `worker.md` 没写 `tools:` 字段就拿到全集，成了潜在的无限递归口子（而 pi 没有任何深度防护）。
...
**超时是常见实现共同的缺陷** —— 说明这是个容易被忽略的点。后果分别是： 永久占 worker slot、 永久阻塞父代理（裸 `await pending_future`）、pi 永久占并发槽位。
...
pi 是唯一做对的，而且是**两端都踩过**才定在 50KB —— 最早只回 100 字符，模型看不懂发生了什么（CHANGELOG #4710）。
...
pi 把 `model:` 写进 frontmatter（`agents.ts`），对它的单机 CLI 场景合适；本项目是多供应商配置的服务端，两者约束不同。
